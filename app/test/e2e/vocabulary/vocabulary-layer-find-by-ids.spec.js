...
describe('Find layer vocabularies by IDs', () => {
    beforeEach(async () => {
    ...
    it('Find vocabularies without ids in body returns a 400 error', async () => {
        ...
        const response = await requester
            .post(`/api/v1/dataset/345/layer/vocabulary/find-by-ids`)
    ...
    it('Find vocabularies with id list containing a vocabulary that exists returns only the listed vocabulary', async () => {
        mockValidateRequestWithApiKey({});
        resourceOne = await new Resource(createResource('rw', 3, 'layer', getUUID())).save();
        resourceTwo = await new Resource(createResource('gfw', 4, 'layer', getUUID())).save();

        const response = await requester
            .post(`/api/v1/dataset/${resourceOne.dataset}/layer/vocabulary/find-by-ids`)
            .set('x-api-key', 'api-key-test')
            .send({
                ids: [resourceOne.id]
            });
    ...
    it('Find vocabularies with id list containing vocabularies that exist returns the listed vocabularies (query param is ignored)', async () => {
        ...
        const response = await requester
            .post(`/api/v1/dataset/${resourceOne.dataset}/layer/vocabulary/find-by-ids?ids=${resourceTwo.id}`)
            .set('x-api-key', 'api-key-test')
            .send({
                ids: [resourceOne.id]
            });
